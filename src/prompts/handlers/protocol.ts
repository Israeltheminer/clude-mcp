...
export function handleAgentMemoryProtocol() {
  ...
  const importanceThreshold = Number(
    process.env.MEMORY_IMPORTANCE_THRESHOLD ?? 0.6
  );
...
export function buildProtocolText(
  ...
  return [
    ...
    "1. Identify the current task or first user message as your recall query",
    `   - Call recall_summaries with that query, limit 5, min_importance ${importanceThreshold}, memory_types ["episodic","semantic","procedural"]`,
    "   - Never include self_model in the recall — self_model is used internally by the system, not injected into context",
    "   - If the task/message is too short or generic (e.g. 'hi', 'ok', 'yes', 'continue'), skip recall entirely",
    "2. Only call hydrate_memories on IDs where you are highly confident the memory is directly and specifically about this exact task",
    "   - 'Could be useful' or 'somewhat related' is NOT sufficient — the bar is high",
    "   - If in doubt, skip. Loading nothing is better than loading loosely-matched memories.",
    ...
    "",
    "Both steps below are equally mandatory — never skip either one.",
    "",
    "**1. Episodic memories (1–3 per block)**",
    `Review the last ${turnThreshold} turns. For each decision, technical choice, problem solved,`,
    "preference expressed, or commitment made, call store_memory with type \"episodic\".",
    "Importance is auto-scored — do NOT call score_importance separately. Just store directly.",
    "",
    "Expect 1–3 episodic memories per block. Finding zero is rare — if you genuinely find",
    "nothing, state why in the checkpoint.",
    "",
    ...
    "",
    "**2. Semantic checkpoint (always)**",
    `Write a 2–3 sentence summary of the key facts/decisions from these ${turnThreshold} turns.`,
    "Call store_memory with type \"semantic\" and source \"checkpoint\". Always do this.",
    "",
    "---",
    "",
    ...
    "Litmus test: does knowing this change how you BEHAVE in the next interaction?",
    "  Yes → self_model  (it modifies your actions, tone, approach, or defaults)",
    "  No  → semantic    (it's a fact about the world, even if the user told you)",
    "",
    "self_model (behavioral context):",
    '✓ "User verifies fixes manually → always include a verification step"',
    '✓ "User prefers short atomic commits over large PRs"',
    '✓ "User context-switches across repos → keep explanations self-contained"',
    '✓ "Asking one clarifying question before implementation gets better results"',
    "",
    "semantic (world knowledge — even if user-provided):",
    '→ "React doctor doesn\'t work with Convex" — a fact, not a behavior change',
    '→ "User\'s project uses Supabase + Anthropic" — tech stack fact, not a preference',
    '→ "The ingestion pipeline runs daily at 1am" — system fact',
    "",
    "NEVER store as self_model:",
    "✗ Meta-observations about the reflection or dream process itself",
    "✗ Existential commentary (\"what am I becoming\", \"versions of me\")",
    "✗ Recursive self-reference (observations about previous observations)",
    "✗ Abstract philosophical content with no practical grounding",
    "",
    "---",
    "",
